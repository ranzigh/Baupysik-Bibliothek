...
def Brennwert(Heiwärmebedarf,TWW_Bedarf, Gaspreis, Investkosten, Wartungskosten, CO2_Gas, CO2_Steuer,JAZ=1):
    Energiebedarf = Heiwärmebedarf + TWW_Bedarf
...
# def Solarthermie_ergänzen(Wärmeerzeuger):
#     Wärmeerzeuger_ST = Wärmeerzeuger.copy()
#     invest = Investkosten + Investkosten_ST
#     for i in range (15):
#         Wärmeerzeuger_ST["Energiekosten"].iloc[i] = Wärmeerzeuger["Energiekosten"].iloc[i] - (Allgemeines["Strompreis"].iloc[i]*Stromertrag_PV*Eigennutzungsgrad_PV)
#         Wärmeerzeuger_ST["Wartungskosten"].iloc[i] = Wärmeerzeuger["Wartungskosten"].iloc[i] + Investkosten_PV*Wartungskosten
#         Wärmeerzeuger_ST["CO2_Ausstoss_real"].iloc[i] = Wärmeerzeuger["CO2_Ausstoss_real"].iloc[i] * (1-Eigennutzungsgrad_PV)
#         Wärmeerzeuger_ST["CO2_Ausstoss_GEG"].iloc[i] = Wärmeerzeuger["CO2_Ausstoss_GEG"].iloc[i] * (1-Eigennutzungsgrad_PV
#     Wärmeerzeuger_ST['CO2_Steuer_real'] = co2steuer(CO2_Steuer, Wärmeerzeuger_ST["CO2_Ausstoss_real"])
#     Wärmeerzeuger_ST['CO2_Steuer_GEG'] = co2steuer(CO2_Steuer, Wärmeerzeuger_ST["CO2_Ausstoss_GEG"])    
#     Wärmeerzeuger_ST['CO2_Ausstoss_Gesamt'] = gesamtco2(Wärmeerzeuger_ST["CO2_Ausstoss_real"])    
#     Wärmeerzeuger_ST['Kosten_pro_Jahr'] = Wärmeerzeuger_ST['Energiekosten'] + Wärmeerzeuger_ST["Wartungskosten"] + Wärmeerzeuger_ST["CO2_Steuer_real"]
#     Wärmeerzeuger_ST['Gesamtkosten'] = gesamtkosten(invest, Wärmeerzeuger_ST['Kosten_pro_Jahr'])
#     return Wärmeerzeuger_ST

def Brennwert_Solar(Heiwärmebedarf,TWW_Bedarf, Gaspreis, Investkosten, Wartungskosten, CO2_Gas, CO2_Steuer, JAZ=1):
    Energiebedarf = Heiwärmebedarf + TWW_Bedarf*0.5
    Brennwert_Solar = pd.DataFrame({'Energiekosten':energiepreis(Gaspreis, Energiebedarf, JAZ),
                      'Wartungskosten':(Investkosten*Wartungskosten),
                      'CO2_Ausstoss_GEG':co2ausstoss(np.full(shape=15,fill_value=CO2_Gas), Energiebedarf, JAZ),
                      })
    Brennwert_Solar['CO2_Ausstoss_Gesamt'] = gesamtco2(Brennwert_Solar["CO2_Ausstoss_GEG"])
    Brennwert_Solar['CO2_Steuer_GEG'] = co2steuer(CO2_Steuer, Brennwert_Solar["CO2_Ausstoss_GEG"])
    Brennwert_Solar['Kosten_pro_Jahr'] = (Brennwert_Solar['Energiekosten']+Brennwert_Solar["Wartungskosten"]+Brennwert_Solar["CO2_Steuer_GEG"])
    Brennwert_Solar['Gesamtkosten'] = gesamtkosten(Investkosten, Brennwert_Solar['Kosten_pro_Jahr'])
    return Brennwert_Solar